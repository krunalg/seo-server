...
(function() {
  ...
  SeoServer = (function() {
    ...
    SeoServer.prototype.buildURL = function(request) {
      var params;
      params = request.query;//_(request.query).pick(this.config.getParamWhitelist);
      if (_(params).isEmpty()) {
      ...
      this.memcachedClient.get(key, function(error, cachedContent) {
        ...
        if (cachedContent && !clearCache) {
          ...
          if(defaultConfig.debugMode){
			fs.writeFile(defaultConfig.snapshotPath+"/"+funcs.getFullDate()+'/'+funcs.getNameOfModule(url)+'.html', cachedContent,function(){
				mkdirp(getDirName(defaultConfig.snapshotPath+"/"+funcs.getFullDate()+'/'+funcs.getNameOfModule(url)+'.html'), function (err) {
    ...
    SeoServer.prototype.logResponseStats = function(request, headers, time) {
      ...
      crawler = request.get('User-Agent')||"UnIdentified Bot";///RedSnapper/.test(request.get('User-Agent')) ? 'Crawler' : 'GoogleBot';
